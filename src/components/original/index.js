...
let riderPool, floorTimer, lobbyTimer;
...
//   let passengers = Math.round(numbers.random.distribution.logNormal(1, passengerMean, passengerSD)[0]);
...
//   let pickUpFloor = Math.floor(Math.random() * (notLobMax - notLobMin + 1) + notLobMin);
//   let dropOffFloor = Math.floor(Math.random() * (notLobMax - notLobMin + 1) + notLobMin);
...
//   let dropOffFloor = Math.round(numbers.random.distribution.normal(1, lobbyMean, lobbySD)[0]);
...
const closestElevator = function (cordsObj) {
...
const tripTime = function (coordinates) {
  ...
  let pickUpFloor = elevator.pickUpFloor;
...
const stillWaiting = function (coordinates) {
  ...
  return closestElevator(waitingObj);
};
...
const setMoving = function (coordinates) {
  ...
  elevator.pickUpFloor = tripStart;
  elevator.dropOffFloor = tripEnd;
};
...
const setStopped = function (coordinates) {
  ...
  elevator.currentFloor = tripEnd;
  elevator.pickUpFloor = null;
  elevator.dropOffFloor = null;
  elevator.riders = 0;
};
...
const stopSystem = function (floorTimer, lobbyTimer) {
